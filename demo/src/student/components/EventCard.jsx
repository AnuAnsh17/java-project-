...
export const EventCard = ({ event, onRegister }) => {
  ...
  return (
    <div className="student-card student-card-hover" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <h3 style={{ fontSize: '1.15rem', color: 'var(--primary-dark)', marginBottom: '0.5rem', cursor: 'pointer' }} onClick={() => navigate(`/student/events/${event.id}`)}>
        {event.title}
      </h3>
      <p style={{ fontSize: '0.85rem', color: 'var(--primary-light)', fontWeight: '600', marginBottom: '0.85rem' }}>
        Organizer: {event.organizer}
      </p>

      <div style={{ fontSize: '0.88rem', color: 'var(--text-secondary)', display: 'flex', flexDirection: 'column', gap: '0.35rem', marginBottom: '1.25rem', flex: 1 }}>
